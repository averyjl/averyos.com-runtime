...
export default {
   ...
   * Also runs the sovereign `kernelAnchorPulse()` for kernel-anchor parity
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    ...
    // Sovereign kernel-anchor pulse — SHA-512 parity + Tier-9 alert bubbling
    const { kernelAnchorPulse, bubbleUpgrade } = await import("../lib/forensics/watchdog");
    const pulseEnv = {
      DB:           env.DB,
    ...
    ctx.waitUntil(
      kernelAnchorPulse(pulseEnv).then((result) => bubbleUpgrade(result, pulseEnv)),
    );
  },
} satisfies {
