 ...
 *
 * Data-driven structural audit for all 114 `app/api` route handlers.
...
// ── All 114 app/api route files (relative to repo root) ──────────────────────
...
describe("App-Router API Routes — structural audit (114 routes)", () => {
  test("route manifest is complete: 114 entries", () => {
    assert.equal(APP_API_ROUTES.length, 114, "Route manifest must enumerate 114 app/api route.ts files");
  });
