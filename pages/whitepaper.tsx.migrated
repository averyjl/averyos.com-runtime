...
/** KaTeX auto-render polling configuration */
const KATEX_POLL_MAX_ATTEMPTS = 20;
const KATEX_POLL_INTERVAL_MS  = 150; // total wait: ~3 seconds

/** Window augmented with KaTeX renderMathInElement after CDN scripts load */
interface KaTeXWindow extends Window {
  renderMathInElement?: (el: Element, opts: object) => void;
}
...
const WhitepaperPage: NextPage<PageProps> = ({ content }) => {
  useEffect(() => {
    ...
    // KaTeX auto-render: process any $...$ or $$...$$ in the rendered content.
    // The KaTeX scripts are loaded deferred; poll until renderMathInElement is ready.
    let attempts = 0;
    const interval = setInterval(() => {
      attempts++;
      const win = window as KaTeXWindow;
      if (typeof win.renderMathInElement === "function") {
        clearInterval(interval);
        const article = document.querySelector("article.truthforce-content");
        if (article) {
          win.renderMathInElement(article, {
            delimiters: [
      ...
      } else if (attempts >= KATEX_POLL_MAX_ATTEMPTS) {
        clearInterval(interval);
      }
  ...
  return (
    <>
      <Head>
        ...
        {/* KaTeX stylesheet — renders $...$ and $$...$$ math expressions */}
        <link
          rel="stylesheet"
          href="https://cdn.jsdelivr.net/npm/katex@0.16.21/dist/katex.min.css"
          crossOrigin="anonymous"
        />
        {/* KaTeX JS + auto-render: processes inline $...$ and display $$...$$ */}
        <script
          defer
