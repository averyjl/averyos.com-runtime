...
$REQUIRED_SECRETS = @(
    ...
    @{ Name = "FIREBASE_PRIVATE_KEY";      Desc = "Firebase service account private key (raw PEM or \n-escaped PEM)"; Required = $false },
    @{ Name = "FIREBASE_DATABASE_URL";     Desc = "Firebase Realtime Database URL";                                   Required = $false },
    @{ Name = "FCM_DEVICE_TOKEN";          Desc = "Creator's FCM device registration token — enables GabrielOS™ FCM HTTP v1 Tier-9 push alerts (wrangler secret put FCM_DEVICE_TOKEN)"; Required = $false },
