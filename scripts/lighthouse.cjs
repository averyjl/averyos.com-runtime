...
function pulse() {
  ...
  try {
    ...
    execSync('git push');
    console.log(`✅ Heartbeat Pulsed at ${timestamp}`);
  } catch (err) {
    console.error("❌ Pulse Blocked. Checking for lattice drift...");
  }
}
...
if (isCI) {
  console.log("☁️ Cloud-Sovereign Mode: Pulse-and-Exit engaged.");
  pulse();
  process.exit(0);
} else {
  server.listen(PORT, () => {
